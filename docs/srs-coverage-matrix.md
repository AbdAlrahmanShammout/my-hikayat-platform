...
**Validation snapshot:** 334 unit suites / 802 tests, 48 e2e suites / 203 tests, including `test/critical-flow.e2e-spec.ts`.
...
| --- | --- |
| §2.2 Author HTTP book create / list / update | `BookService.createBook` / `updateBook` / `listBooks` exist. There is no `POST /author/books` (or author list/update). Tests create books through the service. |
| §2.3 Admin HTTP book management beyond review | Admins can list `in_review`, get, approve, and reject. There is no HTTP to edit metadata, unpublish, or delete a book. |
| §5.1 Reading speed (pages/minute) | Not computed or stored. |
| §5.1 Reflowable time-per-page and time-per-chapter | Sessions store position plus active/idle ms. There are no per-chapter or per-page totals. |
| §6 Paid entitlement vs Stripe period | Entitlement is `status === ACTIVE && plan.kind === MONTHLY_PAID`. It does not check `currentPeriodEnd`. `invoice.payment_failed` is ignored, so `past_due` stays entitled until a canceled-class event. |
| §12.3 Reflowable engagement heatmap | Author/admin heatmaps are spread-only, so reflowable heatmaps are empty. |
...
| §1 Advanced reading-intelligence tracking | Session ingest of active vs idle; visual engagement for fixed-layout | `reading-intelligence.e2e-spec.ts`, `reading-visual-engagement.e2e-spec.ts`, `reading-intelligence.service.spec.ts` | Complete |
| §1 Subscription-based access | Free + monthly paid plans, Stripe checkout/webhooks, entitlement gate | `subscription.e2e-spec.ts`, `subscription-billing.e2e-spec.ts`, `entitlement.e2e-spec.ts` | Complete |
...
| §2.1 Browse published books | `GET /reader/catalog`, `GET /reader/catalog/:id` | `reader-catalog.e2e-spec.ts` | Complete |
...
| §2.2 Author HTTP create / list / update book metadata | `BookService.createBook` / `updateBook` / `listBooks` only; no `POST /author/books` | `book.service.spec.ts` | In Progress |
...
| §2.3 Review then approve or reject books | `GET /admin/books`, `POST /admin/books/:id/approve`, `POST /admin/books/:id/reject` | `book-admin-review.e2e-spec.ts` | Complete |
...
| §2.3 Manage books beyond the review queue | No HTTP to edit, unpublish, or delete books | `book-admin-review.e2e-spec.ts` covers review only | In Progress |
| §2.3 Curated collections CRUD, membership, display order | `admin/collections` create/edit/delete/add/remove/reorder | `admin-collection.e2e-spec.ts` | Complete |
...
| §4.2 Smart Resume reflowable (page / scroll offset / last session) | `PUT/GET /reader/books/:id/progress` | `reading-smart-resume.e2e-spec.ts` | Complete |
...
| §5.1 Reflowable time per page / time per chapter totals | No per-chapter or per-page totals; reflowable heatmap empty | `author-monetization.e2e-spec.ts` (spread heatmap only) | In Progress |
...
| §6 Paid entitlement follows Stripe period / payment_failed | Entitlement ignores `currentPeriodEnd`; `invoice.payment_failed` ignored | `entitlement.service.spec.ts`, `dispatch-stripe-webhook-event.helper.spec.ts` | In Progress |
| §6.1 7-day refund | `POST /reader/billing/refund` | `subscription-refund.e2e-spec.ts` | Complete |
| §7.1 / §7.2 Weighted engagement and book revenue formula | Reflowable: active reading ms × category weight. Fixed-layout: spread `activeDurationMs` × category weight. `visualSceneTimeMs` is stored and shown, not added to the revenue weight (matches the SRS example formula). | `book-engagement.service.spec.ts`, `book-revenue.e2e-spec.ts` | Complete |
...
| §8 JWT auth, authorization, validation, upload checks, webhook verification, rate limits, audit of publishing/admin actions | Auth module, guards, validation pipe, Stripe signature, throttling, `AuditLog` | `auth.e2e-spec.ts`, `http-surface.e2e-spec.ts`, `auth-throttle.e2e-spec.ts`, `subscription-billing.e2e-spec.ts`, `admin-audit.e2e-spec.ts` | Complete |
...
| §12.3 Fixed-layout engagement heatmap | `GET /author/analytics/books/:bookId/heatmap` | `author-monetization.e2e-spec.ts` | Complete |
| §12.3 Reflowable “where users spend time” heatmap | Spread heatmap only; reflowable cells are empty | `author-monetization.e2e-spec.ts` | In Progress |
...
- Collection CRUD and revenue-period calculate are not written to the audit log.
