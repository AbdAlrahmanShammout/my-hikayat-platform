...
**Validation snapshot:** 379 unit spec files, 57 e2e suites, including `test/critical-flow.e2e-spec.ts`. Snapshot after Steps 1–8 and the reflowable page-tracking decision.
...
| §1 Subscription-based access | Free + admin-defined paid Stripe plans, checkout/webhooks, period-end entitlement | `subscription.e2e-spec.ts`, `subscription-billing.e2e-spec.ts`, `entitlement.e2e-spec.ts` | Complete |
...
| §2.1 Subscribe (admin-defined paid plans) | `GET /reader/billing/plans`, `POST /reader/billing/checkout` (`planId`), `POST /webhooks/stripe`, `GET /reader/billing/subscription` | `subscription-billing.e2e-spec.ts`, `plan.service.spec.ts` | Complete |
...
| §2.3 Manage subscription plans | `GET/POST /admin/plans`, `PATCH /admin/plans/:id` (Stripe retrieve for paid price; one free plan) | `plan.service.spec.ts`, `plan.admin.controller.ts` | Complete |
