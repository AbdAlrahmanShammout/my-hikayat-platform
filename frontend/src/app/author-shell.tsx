...
/**
 * Protected author chrome: sidebar on desktop, drawer on small screens.
 */
export function AuthorShell(): JSX.Element {
  ...
  return (
    <div className="min-h-screen bg-background">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:flex md:flex-col">
        <AuthorSidebar onNavigate={closeMobileNav} />
      </aside>
      <MobileAuthorDrawer isOpen={isMobileNavOpen} onClose={closeMobileNav} />
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex flex-col gap-1 p-4" aria-label="Author">
        {AUTHOR_NAV_ITEMS.map((item) => (
          <AuthorNavLink key={item.to} item={item} onNavigate={onNavigate} />
        ))}
      </nav>
    </div>
  );
...
function AuthorNavLink({
  item,
  onNavigate,
}: {
  readonly item: AuthorNavItem;
      ...
      {isOpen ? (
        <>
          ...
          >
            ...
            <AuthorSidebar onNavigate={onClose} />
          </motion.aside>
        </>
      ) : null}
    </AnimatePresence>
...
function AuthorSessionActions(): JSX.Element {
